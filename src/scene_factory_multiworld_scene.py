...
def _spawn_workzone_viz_overlays(
    ...
    *,
    ...
    Args:
        ...
        cone_spacing: meters between consecutive cones; lower = denser.
    ...
    cone_height = 3.0   # ~10 ft — visible from aerial, proportional to lane width
    ...
    def _place_cone(path: str, wx: float, wy: float) -> None:
        try:
            prim = UsdGeom.Cone.Define(stage, path)
            prim.GetRadiusAttr().Set(0.6)
            prim.GetHeightAttr().Set(cone_height)
            prim.GetAxisAttr().Set("Z")
            UsdGeom.XformCommonAPI(prim).SetTranslate(Gf.Vec3d(wx, wy, cone_height / 2))
            _set_color(prim, orange)
        except Exception:
            cube = UsdGeom.Cube.Define(stage, path)
            cube.GetSizeAttr().Set(1.0)
            xapi = UsdGeom.XformCommonAPI(cube)
            xapi.SetTranslate(Gf.Vec3d(wx, wy, cone_height / 2))
            xapi.SetScale(Gf.Vec3f(1.2, 1.2, cone_height))
    ...
    # cone_spacing comes from the function parameter.
    cone_idx = 0
    ...
    while s <= run_len + 0.01:
        ...
        _place_cone(f"{overlay_root}/Cone_{cone_idx:02d}", wx, wy)
        cone_idx += 1
        s += cone_spacing
    ...
    # Back cap (s=run_len): right_offset from cone_spacing to lane_w-cone_spacing
    r = cone_spacing
    while r < lane_w - 0.1:
    ...
    n_taper_steps = max(5, int(taper_len / cone_spacing))
    ...
    print(
        ...
        f"{len(run)} red tiles ({run_len:.1f} m), {cone_idx} cones, "
        f"taper_factor={taper_factor:.1f}, cone_spacing={cone_spacing:.1f} m",
        flush=True,
    )
...
def _build_single_world_roads_only(
    *,
    ...
    builder.build_from_json(
        ...
        parked_if_start_in_goal=False,
        skip_if_start_in_goal=True,
        video_cone_markers=video_cone_markers,
    )
...
def main() -> None:
    ...
    _workzone_viz = bool(getattr(args_cli, "workzone_viz", False))
    if _workzone_viz and int(args_cli.world_count) <= 0:
    ...
    if _workzone_viz:
        ...
        # Axis 2: cone density (cone_spacing in metres)  — visual clarity / material cost
        _taper_designs = [
            dict(taper_factor=0.8, cone_spacing=2.0, label="short-steep  dense"),
            dict(taper_factor=1.5, cone_spacing=3.0, label="standard     dense"),
            dict(taper_factor=0.8, cone_spacing=6.0, label="short-steep  sparse"),
            dict(taper_factor=1.5, cone_spacing=6.0, label="standard     sparse"),
        ]
        _wz_centers: list = []
