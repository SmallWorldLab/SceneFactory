...
def compute_multilane_capacity(
    ...
    *,
    ...
    ``open_lane_ids`` (from ``scene_cfg["zones"]["keepout_boxes"][0]``), and
    ...
    wz = scene_cfg.get("zones", {}) or {}
    boxes = wz.get("keepout_boxes", []) or []
    if not boxes:
        return empty
    box = boxes[0]
...
def sample_multilane_merge_start_goal_pairs(
    ...
    *,
    ...
    Reads the merge contract from `scene_cfg["zones"]["keepout_boxes"][0]`:
    ...
    boxes = wz.get("keepout_boxes", []) or []
    if not boxes:
        raise RuntimeError("scene has no workzone.keepout_boxes for multilane merge OD")
    box = boxes[0]
