...
parser.add_argument("--reward_workzone_cone_collision_dist_m", type=float, default=float(_cfg_value(file_cfg, "env", "reward_workzone_cone_collision_dist_m", 0.5)))
parser.add_argument("--reward_workzone_cone_collision_penalty", type=float, default=float(_cfg_value(file_cfg, "env", "reward_workzone_cone_collision_penalty", -6.0)))
parser.add_argument("--reward_workzone_box_entry_penalty", type=float, default=float(_cfg_value(file_cfg, "env", "reward_workzone_box_entry_penalty", -5.0)))
...
parser.add_argument(
    "--eval_with_cones",
    action=argparse.BooleanOptionalAction,
    default=bool(_cfg_value(file_cfg, "test", "eval_with_cones", False)),
    help="If enabled, register the RandomConeDropper during eval/test mode too "
    "(normally cones are only dropped during training). Requires cone_randomize_num>0 in the env config.",
)
parser.add_argument(
    "--random_od",
    action=argparse.BooleanOptionalAction,
...
parser.add_argument(
    "--video_cone_markers",
    action=argparse.BooleanOptionalAction,
    default=bool(_cfg_value(file_cfg, "video", "video_cone_markers", False)),
...
def _build_env_cfg() -> StudentVehicleMultiAgentGoalEnvCfg:
    ...
    cfg.reward_workzone_cone_collision_dist_m = float(args_cli.reward_workzone_cone_collision_dist_m)
    cfg.reward_workzone_cone_collision_penalty = float(args_cli.reward_workzone_cone_collision_penalty)
    cfg.reward_workzone_box_entry_penalty = float(args_cli.reward_workzone_box_entry_penalty)
    ...
    cfg.video_cone_markers = bool(args_cli.video_cone_markers)
...
def _build_resolved_config(
    ...
    return {
        ...
        "reward": {
            ...
            "workzone_cone_collision_dist_m": float(env_cfg.reward_workzone_cone_collision_dist_m),
            "workzone_cone_collision_penalty": float(env_cfg.reward_workzone_cone_collision_penalty),
            "workzone_box_entry_penalty": float(env_cfg.reward_workzone_box_entry_penalty),
...
def _run_scene_factory_policy_eval(
    ...
    _workzone_worlds = [item for item in completed_items if float(item.get("cone_near_miss_rate", -1.0)) >= 0.0]
    cone_near_miss_rate = (
        float(sum(float(item.get("cone_near_miss_rate", 0.0)) for item in _workzone_worlds) / max(1, len(_workzone_worlds)))
        if _workzone_worlds
        else -1.0
    )
    speed_violation_rate = (
        float(sum(float(item.get("speed_violation_rate", 0.0)) for item in _workzone_worlds) / max(1, len(_workzone_worlds)))
        if _workzone_worlds
        else -1.0
    )
        ...
        [
            {
                ...
                "cone_near_miss_rate": float(item.get("cone_near_miss_rate", -1.0)),
    ...
    summary = {
        ...
        "cone_near_miss_rate": float(cone_near_miss_rate),
    ...
    _wz_str = f" cone_near_miss_rate={summary['cone_near_miss_rate']:.3f} speed_violation_rate={summary['speed_violation_rate']:.3f} box_ever_entered_rate={summary['box_ever_entered_rate']:.3f} box_entry_rate={summary['box_entry_rate']:.3f}" if summary['cone_near_miss_rate'] >= 0.0 else ""
    print(
        f"[INFO][SceneFactory] {test_mode_name} finished. "
...
def main():
    ...
    # ── Cone randomization (activated by cone_randomize_num > 0 in env config) ──
    # Normally cones drop only during training; --eval_with_cones opts eval in too
    # (so you can watch the trained policy interact with the cones it learned on).
    _cone_randomize_num = int(_cfg_value(file_cfg, "env", "cone_randomize_num", 0))
    _cone_randomize_in_eval = bool(getattr(args_cli, "eval_with_cones", False))
    if _cone_randomize_num > 0 and (not eval_test_mode or _cone_randomize_in_eval):
        from src.cone_randomizer import (
            ensure_cone_buffer,
            RandomConeDropper,
            DEFAULT_CONE_SLOTS as _MAX_CONE_SLOTS,
        )
        print(
            f"[INFO][ConeRandomizer] Enabling random cone drops: "
            f"{_cone_randomize_num} cones/env/episode, buffer={_MAX_CONE_SLOTS} slots",
            flush=True,
        )
        ...
        ensure_cone_buffer(base_env, _MAX_CONE_SLOTS)
        _dropper = RandomConeDropper(
            base_env,
            num_cones=_cone_randomize_num,
            seed=int(runner_cfg.seed),
            od_corridor=_od_corridor,
            corridor_width_m=_corridor_width_m,
        )
        base_env._cone_drop_fn = _dropper.drop
        print(
            f"[INFO][ConeRandomizer] RandomConeDropper registered: "
